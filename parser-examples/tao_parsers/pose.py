...
config = yaml.load(open('sample_bodypose2d_model_config.yml'))
...
def pose_plot(display_meta, joint_list, person_to_joint_assoc, limb_thickness = 4, outsize=(1280, 720)):
    ...
    for limb_type in range(NUM_LIMBS):
        for person_joint_info in person_to_joint_assoc:
            ...
            j += 1
            lp.x1 = round(joint_coords[0][0]*outsize[0])
            lp.y1 = round(joint_coords[0][1]*outsize[1])
            lp.x2 = round(joint_coords[1][0]*outsize[0])
            lp.y2 = round(joint_coords[1][1]*outsize[1])
