...
def pose_decode(heatmaps, pafs, param=param):
    upsampFactor = (pafs.shape[0]/heatmaps.shape[0], pafs.shape[1]/heatmaps.shape[1])
    joint_list_per_joint_type = NMS(param, heatmaps, upsampFactor, bool_refine_center = False)
...
if '__main__' == __name__:
    ...
    p.add_argument('im', nargs='?', default='sample_720p.jpg')
    p.add_argument('output', nargs='?', default='output_1.npy')
    p.add_argument('config', nargs='?', default='sample_bodypose2d_model_config.yml')
    args = p.parse_args()
    ...
    output = np.load(args.output, allow_pickle=True)

    pafs = output.item()['pafmap']

    heatmaps = output.item()['heatmap']
...
#    sys.exit(0)
    ...
    for limb_type in range(NUM_LIMBS):
        for person_joint_info in person_to_joint_assoc:
            ...
            plot(joint_coords[:, 0]*1280, joint_coords[:,1]*720)
