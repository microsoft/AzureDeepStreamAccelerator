...
config = yaml.load(stream=open(join(CURR_DIR, './sample_bodypose2d_model_config.yml')), Loader=SafeLoader)
jointEdges = config['jointEdges']
connections = config['connections']

joint_to_limb_heatmap_relationship = [list(map(int, e.split(','))) for e in jointEdges]
...
# purpose.
#map = matplotlib.cm.get_cmap('hsv')
...
def NMS(param, heatmaps, upsampFactor=(1,1), bool_refine_center=False, bool_gaussian_filt=False):
    """
    ...
    for joint in range(NUM_JOINTS):
        ...
        for i, peak in enumerate(peak_coords):
            if bool_refine_center:
                ...
                map_upsamp = zoom(patch, upsampFactor)
...
def pose_plot(display_meta, joint_list, person_to_joint_assoc, limb_thickness = 4, outsize=(1280, 720)):
    j = 0
    ...
    for limb_type in range(NUM_LIMBS):
        for person_joint_info in person_to_joint_assoc:
            ...
            joint_indices = person_joint_info[[limb[0], limb[1]]].astype(int)
            if -1 in joint_indices:
                # Only draw actual limbs (connected joints), skip if not
                # connected
                continue
            # joint_coords[:,0] represents X coords of both joints;
            # joint_coords[:,1], Y coords
            joint_coords = joint_list[joint_indices, 0:2]
            if j >= 16:
                display_meta.num_lines=j
                continue
            lp = display_meta.line_params[j]

            j += 1
            lp.x1 = int(round(joint_coords[0][0]*outsize[0]))
            lp.y1 = int(round(joint_coords[0][1]*outsize[1]))
            lp.x2 = int(round(joint_coords[1][0]*outsize[0]))
            lp.y2 = int(round(joint_coords[1][1]*outsize[1]))
            lp.line_width = limb_thickness
            lp.line_color.set(*[c/255 for c in colors[limb_type]]+[0.5,])
    display_meta.num_lines=j

param = {}
param['thre1'] = 0.05 #config['threshHeat']
param['thre2'] = 0.05 #config['threshVectorScore']
#param['win_size'] = 2 #config['nmsWindowSize']

def pose_decode(heatmaps, pafs, param=param, refine_center=False, gaussian_filt=False):
    upsampFactor = (pafs.shape[0]/heatmaps.shape[0], pafs.shape[1]/heatmaps.shape[1])
    joint_list_per_joint_type = NMS(param, heatmaps, upsampFactor, 
                                    bool_refine_center = refine_center, 
                                    bool_gaussian_filt=gaussian_filt)
    ...
    if joint_list.ndim < 2:             # last frame (partial)
        return [], []
    joint_list[:, 0] /= pafs.shape[1]   # x
    joint_list[:, 1] /= pafs.shape[0]   # y

    return person_to_joint_assoc, joint_list

def get_bboxes_and_lines(joint_list, person_to_joint_assoc, outsize=(1280, 720)):
    global person_joint_info

    def scale_box(rect, sx, sy):
        x, y, w, h = rect
        nw = sx*w
        nh = sy*h    
        x = x + 0.5*(w - nw)
    ...
    for person_joint_info in person_to_joint_assoc:       
        ...
        for limb_type in range(14): 
            ...
            tx1 = joint_coords[0][0]*outsize[0]
            ty1 = joint_coords[0][1]*outsize[1]
            tx2 = joint_coords[1][0]*outsize[0]
            ty2 = joint_coords[1][1]*outsize[1]
            ...
            lines.append( [( int(tx1), int(ty1)), (int(tx2), int(ty2))] )
                
        if x1 != bigVal:
            box = scale_box((x1, y1, x2-x1+1, y2-y1+1), sx=1.2, sy=1.5)
            dets.append( [ box,  lines ]   )
